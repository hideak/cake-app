<template>
  <div class="record-item">
    ...
    <div class="products">
      <div
        v-for="productEntry of record.products"
        :key="productEntry.id"
        class="product"
      >
        <div class="left">
        ...
        <div class="right">
          {{ `R$ ${productEntry.price.toFixed(2).replace(".", ",")}` }}
        </div>
      </div>
    </div>
...
import ServiceEntry from "@/model/product-entry.model";
...
export default defineComponent({
  ...
  props: {
    record: {
      ...
      default: new Record(0, [], new Date(), "00:00", "Details"),
    },
  },
  setup(props) {
    /**
    ...
    const computedTotal = computed(() => {
      const result = props.record.products
